...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, itemClassName = '' }) => (
  <div
    className={`scroll-stack-card relative w-full my-8 p-8 md:p-12 rounded-3xl md:rounded-[40px] shadow-[0_0_30px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform ${itemClassName}`.trim()}
    style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const endElementOriginalTopRef = useRef<number>(0);
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // Use cached original positions for useWindowScroll to avoid transform feedback loop
    const endElementTop = useWindowScroll
      ? endElementOriginalTopRef.current
      : (() => {
          const endElement = scrollerRef.current?.querySelector('.scroll-stack-end') as HTMLElement | null;
          return endElement ? getElementOffset(endElement) : 0;
        })();
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = useWindowScroll
        ? cardOriginalTopsRef.current[i] ?? 0
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = useWindowScroll
            ? cardOriginalTopsRef.current[j] ?? 0
  ...
  useLayoutEffect(() => {
    ...
    if (useWindowScroll) {
      ...
      if (endEl) {
        ...
        endElementOriginalTopRef.current = endRect.top + window.scrollY;
      }
    }

    // Use native scroll + RAF for useWindowScroll mode (reliable fallback)
