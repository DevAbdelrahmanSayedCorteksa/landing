...
const fragmentShader = `
  ...
  void main() {
    ...
    vec4 raw = cppn_fn(uv, 0.1 * sin(0.3 * iTime), 0.1 * sin(0.69 * iTime), 0.1 * sin(0.44 * iTime));

    // Map CPPN pattern to purple color palette
    float lum = dot(raw.rgb, vec3(0.299, 0.587, 0.114));
    vec3 deepDark   = vec3(0.06, 0.02, 0.14);
    vec3 darkPurple = vec3(0.20, 0.08, 0.38);
    vec3 midPurple  = vec3(0.40, 0.18, 0.72);
    vec3 primary    = vec3(0.55, 0.30, 0.95);
    vec3 lightGlow  = vec3(0.75, 0.55, 1.0);

    vec3 color = mix(deepDark, darkPurple, smoothstep(0.05, 0.25, lum));
    color = mix(color, midPurple, smoothstep(0.25, 0.45, lum));
    color = mix(color, primary, smoothstep(0.45, 0.65, lum));
    color = mix(color, lightGlow, smoothstep(0.65, 0.85, lum));
